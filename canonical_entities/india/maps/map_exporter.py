...
Usage:
    python map_exporter.py --state Assam [--level all] [--out-dir Geojson]

If --state is omitted, the script prompts for it interactively.
...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "Geojson"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download India admin boundaries from the NIC admin2024 service.")
    parser.add_argument("--state", help="State name (e.g. Assam, Odisha). Prompted for if omitted.")
    parser.add_argument("--level", choices=sorted(VALID_LEVELS), default="all", help="Boundary level to download.")
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=DEFAULT_OUTPUT_DIR,
        help=f"Directory to write GeoJSON/CSV output to (default: {DEFAULT_OUTPUT_DIR}).",
    )
    return parser.parse_args(argv)


def run(state_input: str, level: str, outdir: Path) -> int:
    ...
    if not state_fc["features"]:
        print(f'ERROR: No match for state="{state_input}"')
        try:
            valid = fetch_distinct_state_names()
            print("Valid state names include:")
            for name in valid:
                print(f"  - {name}")
        except Exception:
            print("Also failed to fetch valid state names.")
        return 1
...
def main(argv: list[str] | None = None) -> int:
    ...
    state_input = args.state if args.state else input("Enter your state name: ").strip()
    return run(state_input, args.level, args.out_dir)
